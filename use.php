<?php
include("./header.php");
?>
<body>
<div id="formWrap">
  <div class="mainimg">
    <div class="pc">
        <img src="img/howToUse_main_PC.png" alt="使い方PCメインビジュアル">
    </div>
    <div class="sp">
        <img src="img/howToUse_main_SP.png" alt="使い方SPメインビジュアル">
    </div>
  </div>		
<section id="2">
<h2 class="center"><span class="bold__01 wf-roundedmplus1c">FollowersCountの特徴</span></h2>
<div class="container center">
<div class="row">
    <div class="col span-4">
		<img src="img/form_00.png" alt="使い方1">
		<h3>フォームに目標を入力</h3>
		<p>フォームに【名前】、【メールアドレス】、【目標値】を入力するだけ！</p>
    </div>
    <div class="col span-4">
		<img src="img/use_00.png" alt="使い方2">
		<h3>入力した目標がメールで届く</h3>
		<p>目標を入力していただいたあと「あれ？目標、何にしたっけ？」を防止！お手元にメールでお知らせが届きます。</p>
    </div>
    <div class="col span-4">
		<img src="img/use_01.png" alt="使い方3">
		<h3>困った時にすぐ聞ける！</h3>
		<p>【お問合せ】を選択すれば、メールで質問OKです。</p>
    </div>
    </div>
	</div>
</section>
<div class="toForm__button">
  <a class="toForm__button__a" href="./index.php"><span class="toForm__button__span">目標値を入力する</span></a>
</div>
<section id="3" class="blue_back">
<h2 class="center"><span class="bold__01 wf-roundedmplus1c">もうスプレッドシートに入力しなくていい…だと！？</span></h2>
<div class="container">
<div class="row flow">
    <div class="col span-3">
		<img src="img/comingsoon_00.png" alt="使い方4">
    </div>
    <div class="col span-9"><h3>お名前を入力(※必須項目)</h3><br /><p>あなたのお名前を入力してください。</p>
    </div>
    </div>	
<div class="row flow">
    <div class="col span-3">
		<img src="img/comingsoon_00.png" alt="使い方5">
    </div>
    <div class="col span-9"><h3>mailアドレスを入力(※必須項目)</h3><br /><p>あなたのメールアドレスを入力してください。数分以内に入力内容の確認メールが届きます。これで安心ですね！</p>
    </div>
    </div>
<div class="row flow">
    <div class="col span-3">
		<img src="img/comingsoon_00.png" alt="使い方6">
    </div>
    <div class="col span-9"><h3>目標値を入力(※必須項目)</h3><br /><p>今週の目標を入力してください。スプレッドシートに入力しようとすると、どうしてもPC環境になってしまいます。これならスマホから入力してもらえます。</p>
    </div>
    </div>
    <div class="row flow">
    <div class="col span-3">
		<img src="img/comingsoon_00.png" alt="使い方5">
    </div>
    <div class="col span-9"><h3>メッセージ(※任意・非公開)</h3><br /><p>任意でメッセージを入力することができます。(非公開)意気込みや、ご質問などございましたらどうぞ。</p>
    </div>
    </div>
	</div>
  <div class="toForm__button">
  <a class="toForm__button__a" href="./index.php"><span class="toForm__button__span">目標値を入力する</span></a>
  </div>
</section>
<?php
include("./footer.php");
?>